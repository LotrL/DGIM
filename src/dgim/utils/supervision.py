...
def spvs_coarse(data, config):
    ...
    NOTE:
        - for scannet dataset, there are 3 kinds of resolution {i, c, f}
        - for megadepth dataset, there are 4 kinds of resolution {i, i_resize, c, f}
    """
    # 1. misc
    device = data['image0'].device
    N, _, H0, W0 = data['image0'].shape
    _, _, H1, W1 = data['image1'].shape
    scale = config['DGIM']['RESOLUTION'][0]
...
def spvs_fine(data, config):
    ...
    N, _, H0, W0 = data['image0'].shape
    _, _, H1, W1 = data['image1'].shape
    scale = config['DGIM']['RESOLUTION'][1]
    scale0 = scale * data['scale0'][:, None] if 'scale0' in data else scale
    scale1 = scale * data['scale1'][:, None] if 'scale1' in data else scale
    h0, w0, h1, w1 = map(lambda x: x // scale, [H0, W0, H1, W1])
    scale_f_c = config['DGIM']['RESOLUTION'][0] // config['DGIM']['RESOLUTION'][1]
    W_f = config['DGIM']['FINE_WINDOW_SIZE']
