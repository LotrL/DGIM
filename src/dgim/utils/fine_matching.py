...
class FineMatching(nn.Module):
    ...
    def __init__(self, config):
        ...
        dim_f = config['backbone']['block_dims'][0]
    ...
    def forward(self, feat_f0_unfold, feat_f1_unfold, data):
        ...
        data.update(**self.get_fine_match(conf_matrix_fine, feat_f0_unfold, feat_f1_unfold, data))

    @torch.no_grad()
    def get_fine_match(self, conf_matrix_fine, feat_f0_unfold, feat_f1_unfold, data):
        """
        Args:
        ...
        mkpts0_f_train = (mkpts0_c_scaled_to_f[b_ids] + mkpts0_f_window - (W_f // 2) + sub_ref0) * scale0
        mkpts1_f_train = (mkpts1_c_scaled_to_f[b_ids] + mkpts1_f_window - (W_f // 2) + sub_ref1) * scale1
