...
class AG_RoPE_EncoderLayer(nn.Module):
    def __init__(self,
        ...
        self.aggregate = nn.Sequential(
            nn.MaxPool2d(kernel_size=int(self.agg_size ** 0.5), stride=int(self.agg_size ** 0.5)),
            nn.Conv2d(d_model, d_model, kernel_size=int(self.agg_size ** 0.5), stride=int(self.agg_size ** 0.5),
                      padding=0, bias=False, groups=d_model)
        )
        if self.rope:
    ...
    def forward(self, x, source, x_mask=None, source_mask=None):
        ...
        source = self.norm1(self.aggregate(source).permute(0, 2, 3, 1))  # [N, H, W, C]

        if x_mask is not None:
            if x_mask.dtype != torch.bool:
                x_mask = x_mask.bool()
            max_pool = nn.MaxPool2d(kernel_size=self.agg_size,
                                    stride=self.agg_size) if self.agg_size != 1 else nn.Identity()
            source_mask = max_pool(source_mask.float()).bool()
...
class LocalFeatureTransformer(nn.Module):
    ...
    def __init__(self, config):
        ...
        self_layer = AG_RoPE_EncoderLayer(config['d_model'], config['nhead'], config['agg_size'], config['no_flash'],
                                          config['rope'], config['npe'], self.fp32)
        cross_layer = AG_RoPE_EncoderLayer(config['d_model'], config['nhead'], config['agg_size'], config['no_flash'],
                                           False, config['npe'], self.fp32)

        self.layers = nn.ModuleList()
        for layer_name in self.layer_names:
            if layer_name == 'self':
                self.layers.append(copy.deepcopy(self_layer))
            elif layer_name == 'cross':
                self.layers.append(copy.deepcopy(cross_layer))
    ...
    def forward(self, feat0, feat1, mask0=None, mask1=None):
        ...
        if bs == 1 and mask0 is not None and mask1 is not None:
            ...
            mask_h0, mask_w0, mask_h1, mask_w1 = mask0[0].sum(-2)[0], mask0[0].sum(-1)[0], mask1[0].sum(-2)[0], \
            mask1[0].sum(-1)[0]
            mask_h0, mask_w0, mask_h1, mask_w1 = mask_h0, mask_w0, mask_h1, mask_w1
        ...
        for i, (layer, name) in enumerate(zip(self.layers, self.layer_names)):
            ...
            if name == 'self':
                ...
                feat0_temp = layer(feat0, feat1, mask0, mask1)
                feat1 = layer(feat1, feat0, mask1, mask0)
                feat0 = feat0_temp
            else:
                raise KeyError
